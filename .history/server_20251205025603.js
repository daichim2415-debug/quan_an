...
// --- 1. THÊM ĐOẠN NÀY LÊN ĐẦU FILE (Cùng chỗ khai báo express, mysql...) ---
const multer = require('multer');
const path = require('path');
...
// --- 2. SỬA LẠI API THÊM MÓN (Dùng upload.single('hinh_anh')) ---
app.post('/api/mon-an', upload.single('hinh_anh'), (req, res) => {
    const { ten_mon, gia, mo_ta } = req.body;
    // Nếu có file ảnh thì lấy đường dẫn, nếu không thì để rỗng
    const hinh_anh = req.file ? '/uploads/' + req.file.filename : '';
...
// --- 3. SỬA LẠI API SỬA MÓN (Cũng dùng upload.single) ---
app.put('/api/mon-an/:id', upload.single('hinh_anh'), (req, res) => {
    const { ten_mon, gia, mo_ta, hinh_anh_cu } = req.body; // hinh_anh_cu là đường dẫn ảnh cũ nếu không chọn ảnh mới
    const { id } = req.params;
    
    // Nếu user chọn ảnh mới -> dùng ảnh mới. Nếu không -> giữ ảnh cũ
